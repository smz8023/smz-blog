...
class LinValidator {
    ...
    async _check(key, alias = {}) {
        ...
        if (!result.pass) {
            ...
            const msg = `${isCustomFunc ? '' : ''}${result.msg}`
...
class RuleField {
    ...
    validate(field) {
        ...
        if (field == null) {
            ...
            const defaultMessage =this. _hasDefaultMessage()
            if (allowEmpty) {
                return new RuleFieldResult(true, '', defaultValue)
            } else {
                return new RuleFieldResult(false, defaultMessage||'字段是必填参数')
            }
        }
